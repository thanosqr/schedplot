...
start(Fun, FolderName, CoreN, Flags) ->
    ...
    io:put_chars(FP, "."),			 
    io:nl(FP),
    io:write(FP, lists:member(trace_mfa, Flags)),
    io:put_chars(FP, "."),
    ok=file:close(FP),
        ...
        {trace_ts,PID,IO,SID,MFA,Time} ->
            case MFA of
                ...
                _ ->
                    ...
                    TracerPID ! trim(Mode,{IO,{PID,MFA,Time}})
            end,
            master_tracer(PIDs, GC_PID, Mode);

        {trace_ts,PID,SE,_GC_Info,Time} ->
            GC_PID ! trim(Mode, {SE, {PID, {gc,gc,0}, Time}}),
            master_tracer(PIDs, GC_PID, Mode);

        {PID,exit} ->
            fwd_rest(PIDs),
            _ = [P ! self() || P <- array:to_list(PIDs)],
            _ = [receive ok -> ok end || _ <- array:to_list(PIDs)],
            PID ! trace_stored
    end.		

trim(true, Msg) -> Msg;
trim(false, {InOut, {_, _, Time}}) -> {InOut, {Time}}.     
